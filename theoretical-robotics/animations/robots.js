...
class Robot {
  ...
  draw(ctx) {
    ...
    ctx.transform(1, 0, 0, -1, w / 2, ctx.canvas.height - 10 * scale_x);
...
function circle(center, radius) {
  ...
  path.ellipse(center.x, center.y, radius, radius, 0, 0, 2 * Math.PI);
  return path;
}
...
function getContext(elementId) {
  ...
  if (!context) {
    throw new Error(
      "Could not create 2D canvas context for element ID: " + elementId
    );
  }
...
class Visualization {
   ...
   * @param {string} q2_id
   */
  addRobotControl(robot, q1_id, q2_id) {
   ...
   * @param {string} canvas_id
   */
  addWorkSpace(robot, canvas_id) {
    const context = getContext(canvas_id);

    const workspace = createWorkspace(robot);

    const self = this;

    this.draw_functions.push(() => {
      robot.drawWorkSpace(context, workspace);
    });
  }
...
function robot1d() {
  ...
  vis.addRobot(robot, "robot1d");
  vis.addRobotControl(robot, "robot1d_q1", "robot1d_q1");
...
function robot2d() {
  const robot = new Robot();
  const vis = new Visualization();
  vis.addRobot(robot, "robot2d");
  vis.addRobotControl(robot, "robot2d_q1", "robot2d_q2");
...
function robot2d_config() {
  const robot = new Robot();
  const vis = new Visualization();
  vis.addRobot(robot, "robot2dc");
  vis.addRobotControl(robot, "robot2dc_q1", "robot2dc_q2");
  vis.addConfigSpace(robot, "robot2dc_config", "robot2dc_q1", "robot2dc_q2");

  vis.nextFrame();
}

function robot2d_workspace() {
  const robot = new Robot();
  const vis = new Visualization();
  vis.addRobotControl(robot, "robot2d_ws_q1", "robot2d_ws_q2");
  vis.addConfigSpace(
    robot,
    "robot2d_ws_config",
    "robot2d_ws_q1",
  ...
  vis.addWorkSpace(robot, "robot2d_ws_work");

  // TODO: Move to the visualization class
  const q1Input = /** @type{HTMLInputElement} */ (
    document.getElementById("robot2d_ws_q1")
  );
  const q2Input = /** @type{HTMLInputElement} */ (
    document.getElementById("robot2d_ws_q2")
  );

  const sampleWorkspaceButton = /** @type{HTMLButtonElement} */ (
    document.getElementById("sampleWorkspace")
  );
  sampleWorkspaceButton.addEventListener("click", () => {
    ...
    function nextConfiguration() {
      ...
      q1Input.dispatchEvent(new Event("input"));
